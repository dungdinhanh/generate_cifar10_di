...
def get_images(net, net_name, bs=256, i_time=0, epochs=1000, idx=-1, var_scale=0.00005,
               net_student=None, prefix=None, competitive_scale=0.00, train_writer=None, global_iteration=None,
               use_amp=False,
    ...
    for epoch in range(epochs):
        ...
        if epoch % 100 == 0 and epoch > 0:
            for ids in range(bs):
                save_image(inputs.data[ids].clone(),
                           './training/{}/{}/{}.png'.format(net_name, epoch, bs * i_time + ids))
    ...
    if idx == 0:
        print('Teacher correct out of {}: {}, loss at {}'.format(bs, predicted_teach.eq(targets).sum().item(),
...
if __name__ == "__main__":
    ...
    parser.add_argument('--iters_mi', default=1001, type=int, help='number of iterations for model inversion')
    ...
    parser.add_argument('--teacher_weights', default='checkpoint/state_dicts/resnet34.pt', type=str,
                        help='path to load weights of the model')
    parser.add_argument('--i_start', default=0, type=int)
    parser.add_argument('--net', default='resnet34', type=str)

    args = parser.parse_args()
    net_name = args.net
    print("loading %s" % net_name)

    for i in range(100, args.iters_mi, 100):
        os.makedirs("./training/{}/{}".format(net_name, i), exist_ok=True)
    if net_name == "resnet50":
        net_teacher = resnet50()
    elif net_name == "resnet34":
        net_teacher = resnet34()
    elif net_name == "resnet18":
        net_teacher = resnet18()
    net_student = resnet18()
    num_params = sum(p.numel() for p in net_teacher.parameters() if p.requires_grad)
    ...
    inputs = get_images(net=net_teacher, i_time=args.i_start, net_name=net_name, bs=args.bs, epochs=args.iters_mi,
                        idx=batch_idx,
                        net_student=None, prefix=prefix, competitive_scale=args.cig_scale,
